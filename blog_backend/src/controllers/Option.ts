...
class Option extends BaseController {
  ...
  async get({ name }: any) {
    ...
    if (!option) {
      ...
      this.statusCode(GET_SUCCESS).message('Success').status(true).data('option', option).send()
    }
  }
  ...
  async all() {
    ...
    else this.status(true).statusCode(GET_SUCCESS).data('options', options).send()
  }
  ...
  async create({ name, value, description, isPublic }: any) {
    ...
    try {
      ...
      this.statusCode(POST_SUCCESS).message('Option created').status(true).data('option', created)
    } catch (error) {
  ...
  async update({ id, name, value, description, isPublic }: any): Promise<void> {
    ...
    else this.status(true).statusCode(POST_SUCCESS).message('Option updated').data('update', updated).send()
  }
  ...
  async delete({ id }: any): Promise<void> {
    ...
    else this.status(true).statusCode(POST_SUCCESS).message('Option deleted').data('deleted', deleted).send()
  }
  ...
  async publics() {
    ...
    if (!options) {
      ...
      this.statusCode(GET_SUCCESS).message('Success').status(true).data('options', options)
    }
    this.send()
  }
}

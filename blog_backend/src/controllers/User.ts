...
class User extends BaseController {
  ...
  async get({ uid }: any): Promise<void> {
    ...
    else this.status(true).statusCode(GET_SUCCESS).data('user', user).send()
  }
  ...
  async getProfile({ uid }: any): Promise<void> {
    ...
    else this.status(true).data('userProfile', userProfile).statusCode(GET_SUCCESS).send()
  }
  ...
  async getDetails({ uid }: any): Promise<void> {
    ...
    if (!user) this.status(false).statusCode(NOTFOUND).message('User not found').send()
      ...
      this.status(true).statusCode(GET_SUCCESS).data('user', details).send()
    }
  }
  ...
  async all() {
    ...
    else this.status(true).statusCode(GET_SUCCESS).data('users', users).send()
  }
  ...
  async changePin({ uid, otp, pin }: any) {
    ...
    this.statusCode(POST_SUCCESS).success('Pin changed').data('userInfo', updated).send()
  }
  ...
  async deleteProfile({ uid }: any): Promise<void> {
    ...
    else this.status(true).statusCode(POST_SUCCESS).message('User Profile deleted').data('deleted', deleted).send()
  }
}

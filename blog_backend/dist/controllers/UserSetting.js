...
class UserSetting extends BaseController_1.default {
    ...
    async get({ name, uid }) {
        ...
        if (!userSettingModel)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('userSetting', userSettingModel).send();
    }
    async all() {
        ...
        this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('userSettings', userSettings).send();
    }
    async create({ uid }) {
        await this.ownerAndAdminAccess(uid || this?.user?._id);
    ...
    async reset({ uid }) {
        ...
        if (!recreated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).message('User setting reseted').data('settings', recreated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('User setting deleted').data('deleted', deleted).send();
    }
}

...
class Option extends BaseController_1.default {
    ...
    async get({ name }) {
        ...
        if (!option) {
            ...
            this.statusCode(statusCodeConstants_1.GET_SUCCESS).message('Success').status(true).data('option', option).send();
        }
    }
    async all() {
        ...
        if (!options)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('options', options).send();
    }
    async create({ name, value, description, isPublic }) {
        ...
        try {
            ...
            this.statusCode(statusCodeConstants_1.POST_SUCCESS).message('Option created').status(true).data('option', created);
        }
        catch (error) {
    ...
    async update({ id, name, value, description, isPublic }) {
        ...
        if (!updated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Option updated').data('update', updated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Option deleted').data('deleted', deleted).send();
    }
    async publics() {
        ...
        if (!options) {
            ...
            this.statusCode(statusCodeConstants_1.GET_SUCCESS).message('Success').status(true).data('options', options);
        }
        this.send();
    }
}

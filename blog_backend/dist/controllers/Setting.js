...
class Setting extends BaseController_1.default {
    ...
    async get({ id }) {
        ...
        if (!found)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('setting', found).send();
    }
    async all() {
        ...
        if (!settings)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('settings', settings).send();
    }
    async create({ name, value, category }) {
        ...
        if (!created)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Setting created').data('created', created).send();
    }
    async update({ id, name, value, category }) {
        ...
        if (!updated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Setting updated').data('update', updated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Setting deleted').data('deleted', deleted).send();
    }
}

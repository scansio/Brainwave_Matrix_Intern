...
class State extends BaseController_1.default {
    ...
    async get({ id }) {
        ...
        if (!state)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('State', state).send();
    }
    async all() {
        ...
        if (!states)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('states', states).send();
    }
    async create(obj) {
        ...
        if (!created)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('State created').data('created', created).send();
    }
    async update(obj) {
        const definedValues = (0, common_1.getDefinedValuesFrom)(obj);
        ...
        if (!updated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('State updated').data('update', updated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('State deleted').data('deleted', deleted).send();
    }
}

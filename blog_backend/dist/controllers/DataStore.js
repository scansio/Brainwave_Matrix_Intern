...
class DataStore extends BaseController_1.default {
    ...
    async all({ store }) {
        ...
        if (!found)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('data', found).send();
    }
    async count({ store }) {
        if (!this._normalAccess) {
            await this.adminAccess();
        ...
        this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('count', count).send();
    }
    async sum({ store, field }) {
        ...
        this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('sum', sum).send();
    }
}

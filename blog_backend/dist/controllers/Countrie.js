...
class Countrie extends BaseController_1.default {
    ...
    async get({ id }) {
        ...
        if (!countrie)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('countrie', countrie).send();
    }
    async all() {
        ...
        if (!countries)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('countries', countries).send();
    }
    async create(obj) {
        ...
        if (!created)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Countrie created').data('created', created).send();
    }
    async update(obj) {
        const definedValues = (0, common_1.getDefinedValuesFrom)(obj);
        ...
        if (!updated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Countrie updated').data('update', updated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Countrie deleted').data('deleted', deleted).send();
    }
}

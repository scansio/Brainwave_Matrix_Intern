...
class Migration extends BaseController_1.default {
    ...
    async names() {
        this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('names', Object.keys(this.migrationModel.models)).send();
    }
    async get({ modelName }) {
    ...
    async all() {
        ...
        if (!models || models.length < 1)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('models', models).send();
    }
    async backup() {
        const backupData = await this.migrationModel.backup();
        this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('backup', backupData).send();
    }

...
class SettingCategory extends BaseController_1.default {
    ...
    async get({ id }) {
        ...
        if (!found)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('settingCategory', found).send();
    }
    async all() {
        ...
        if (!settingCategories)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('settingCategories', settingCategories).send();
    }
    async create({ name }) {
        ...
        if (!created)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Category created').data('created', created).send();
    }
    async update({ id, name }) {
        ...
        if (!updated)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Category updated').data('update', updated).send();
    }
    async delete({ id }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('Category deleted').data('deleted', deleted).send();
    }
}

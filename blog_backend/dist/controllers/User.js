...
class User extends BaseController_1.default {
    ...
    async login({ email, password }) {
        ...
        if (user == null) {
            if (isAdmin) {
                ...
                this.statusCode(statusCodeConstants_1.BAD_AUTHENTICATION)
            ...
            if (isAdmin || user.status == constants_1.ACTIVE) {
                ...
                this.status(true).message('Logged in').data('token', token).uid(user._id).send();
            }
                ...
                this.statusCode(statusCodeConstants_1.BAD_AUTHENTICATION).errorCode(errorCodeConstants_1.UNVERIFIED).error('Please verify your email').send();
            }
        }
    }
    async get({ uid }) {
        ...
        if (!user)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('user', user).send();
    }
    async getProfile({ uid }) {
        ...
        if (!userProfile)
            ...
            this.status(true).data('userProfile', userProfile).statusCode(statusCodeConstants_1.GET_SUCCESS).send();
    }
    ...
    async all() {
        ...
        if (!users)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('users', users).send();
    }
    async profileAll() {
        await this.adminAccess();
        ...
        if (!userProfiles)
            ...
            this.status(true).statusCode(statusCodeConstants_1.GET_SUCCESS).data('userProfiles', userProfiles).send();
    }
    ...
    async update() {
        ...
        const { uid = this.user._id, firstname, lastname, email, role, bio, country, state, phone, status, } = this.req.body;
        ...
        const definedValues = (0, common_1.getDefinedValuesFrom)({
            ...
            role: (await this.adminAccess(false))
                ? role == constants_1.AuthenticationLevel.DEVELOPER
                    ? (await this.developerAccess(false))
                        ? constants_1.AuthenticationLevel.DEVELOPER
                        : constants_1.AuthenticationLevel.END_USER
                    : role == constants_1.AuthenticationLevel.ADMIN
                        ? constants_1.AuthenticationLevel.ADMIN
                        : constants_1.AuthenticationLevel.END_USER
                : constants_1.AuthenticationLevel.END_USER,
        ...
        this.statusCode(statusCodeConstants_1.POST_SUCCESS)
            ...
            .data('userInfo', { ...userProfileUpdate, ...userInfoUpdate })
            .send();
    }
    ...
    async changePin({ uid, otp, pin }) {
        ...
        this.statusCode(statusCodeConstants_1.POST_SUCCESS).success('Pin changed').data('userInfo', updated).send();
    }
    ...
    async delete({ uid }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('User account deleted').data('deleted', deleted).send();
        }
    }
    async deleteProfile({ uid }) {
        ...
        if (!deleted)
            ...
            this.status(true).statusCode(statusCodeConstants_1.POST_SUCCESS).message('User Profile deleted').data('deleted', deleted).send();
    }
}
